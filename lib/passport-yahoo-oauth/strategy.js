...
Strategy.prototype.userProfile = function(token, tokenSecret, params, done) {
  this._oauth.get('https://social.yahooapis.com/v1/user/' + params.xoauth_yahoo_guid + '/profile?format=json', token, tokenSecret, function (err, body, res) {
    ...
    try {
      ...
      if (json.profile.emails) {
        profile.emails = [];

        json.profile.emails.forEach(function(email) {
          var output = {};

          output.value = email.handle;

          if (email.primary) {
            output.primary = email.primary;
          }

          if (email.type) {
            output.type = email.type.toLowerCase();
          }

          profile.emails.push(output);
        });
      }
